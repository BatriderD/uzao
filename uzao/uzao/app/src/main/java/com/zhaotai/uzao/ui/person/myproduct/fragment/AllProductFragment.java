...
public class AllProductFragment extends BaseFragment implements MyProductFragmentContract.View,
    ...
    public void initView() {
        ...
        //设置空页面
        mAdapter.setEmptyStateView(_mActivity, R.mipmap.ic_state_empty_1, "还没有任何商品，快快去设计哦", getString(R.string.empty_btn), new BtnOnClickListener() {
            @Override
    ...
    public void onItemChildClick(BaseQuickAdapter adapter, View view, final int position) {
        ...
        switch (view.getId()) {
            ...
            case R.id.tv_my_product_bottom_middle_button:
                if ("published".equals(info.status)) {
                    //下架
            ...
            case R.id.tv_my_product_bottom_right_button:
                //未审核
                if ("unReviewed".equals(info.status)) {
                    //提交审核
                    showProgressDialog();
                    mPresenter.submitMyProductToCheck(info.sequenceNBR, info.isTemplate, position);
                }
                //未发布
                if ("unPublished".equals(info.status)) {
                    //立即发布
                    showProgressDialog();
                    mPresenter.publishImmediately(info.sequenceNBR, info.isTemplate, position);
                }
                //已发布
                if ("published".equals(info.status)) {
                    //查看详情
                ...
                //审核未通过
                if ("unApproved".equals(info.status)) {
                    //编辑
                    ModifyTemplateDetailActivity.launch(_mActivity, info.sequenceNBR, position);
                }
                break;
