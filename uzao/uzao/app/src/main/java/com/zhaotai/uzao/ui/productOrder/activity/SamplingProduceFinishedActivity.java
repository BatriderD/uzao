 ...
 * Time: 2017/8/23
 * Created by LiYou
 * Description : 打样加大货  大货 -- 已完成
 */

public class SamplingProduceFinishedActivity extends BaseActivity implements ProductOrderContract.ProductOrderDetailView {
    ...
    @BindView(R.id.tv_product_order_logistics)
    TextView mLogistics;
    ...
    //完成时间
    @BindView(R.id.tv_product_order_detail_finished_time)
    ...
    public static void launch(Context context, String orderNo,String orderType) {
        Intent intent = new Intent(context, SamplingProduceFinishedActivity.class);
    ...
    protected void initView() {
        setContentView(R.layout.activity_product_order_detail_sampling_produce_finished);
        mTitle.setText("订单详情");
        mRightBtn.setVisibility(View.VISIBLE);
        mRightBtn.setImageResource(R.drawable.service);
        mBtnSamplingLogistics.setText("物流信息");
        mBtnProduceLogistics.setText("物流信息");
    }
    ...
    public void showProductOrderDetail(ProductOrderDetailBean productOrderDetail) {
        ...
        //完成时间
        mFinishedTime.setText(TimeUtils.millis2String(Long.parseLong(productOrderDetail.orderDates.confirmReceive)));
        ...
        switch (orderType){
            case OrderStatus.PRODUCE:
                findViewById(R.id.rl_product_order_detail_sampling_receive_time).setVisibility(View.GONE);
