package com.zhaotai.uzao.utils.luban;
...
public class Luban implements Handler.Callback {
    ...
    private File getImageCacheFile(Context context) {
        if (getImageCacheDir(context) != null) {
            return new File(getImageCacheDir(context) + "/" + System.currentTimeMillis() + (int) (Math.random() * 1000) + ".png");
        }
        return null;
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    File result = new Engine(file, getImageCacheFile(context), isDefault, maxSize).compress();
    ...
    private File get(final Context context) throws IOException {
        return new Engine(file, getImageCacheFile(context), isDefault, maxSize).compress();
    }
