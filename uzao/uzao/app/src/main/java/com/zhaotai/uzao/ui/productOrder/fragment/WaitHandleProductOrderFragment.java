...
public class WaitHandleProductOrderFragment extends BaseFragment implements ProductOrderContract.AllView, BaseQuickAdapter.RequestLoadMoreListener
    ...
    public static WaitHandleProductOrderFragment newInstance() {
        return new WaitHandleProductOrderFragment();
    }
    ...
    public void initData() {
        showLoading();
        mPresenter.getWaitHandleProductOrderList(0, true);
    }
            ...
            case R.id.tv_product_order_bottom_first_button:
                switch (info.orderStatus) {
                    ...
                    case OrderStatus.WAIT_FIRST_PAY://待付首款
                        WaitFirstPayProductOrderActivity.launch(_mActivity, info.orderNo, info.orderType, position, false);
                        break;
                    ...
                    case OrderStatus.WAIT_LAST_PAY://待付尾款
                        switch (info.orderType) {
                            case OrderStatus.SAMPLING_PRODUCE:
                                SamplingProduceWaitLastPayActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                            case OrderStatus.PRODUCE:
                                ProduceWaitLastPayActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                        }
                        break;
            ...
            case R.id.tv_product_order_bottom_second_button:
                switch (info.orderType) {
                    case OrderStatus.SAMPLING_PRODUCE://打样加大货
                        switch (info.orderStatus) {
                            ...
                            case OrderStatus.WAIT_PRODUCE_RECEIVE://大货样确认
                                SamplingProduceProduceSamplingReceiveActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                            case OrderStatus.WAIT_RECEIVE://待收货
                                SamplingProduceWaitReceiveActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                        }
                        break;
                    case OrderStatus.PRODUCE://大货
                        switch (info.orderStatus) {
                            case OrderStatus.WAIT_PRODUCE_RECEIVE://大货样确认
                                ProduceSampleReceiveActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                            case OrderStatus.WAIT_RECEIVE://待收货
                                ProduceWaitReceiveActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                        }
                        break;
                    case OrderStatus.SAMPLING://打样
                        switch (info.orderStatus) {
                            case OrderStatus.WAIT_RECEIVE:
                                SampleWaitReceiveActivity.launch(_mActivity, info.orderNo, position, false);
                                break;
                        }
                        break;
                }
     ...
     * 确认收货 -- 样品的确认收货
     *
     * @param type "produce"大货样 "sampling" 打样
     */
    ...
    public void onMessageEvent(WaitHandleSampleProduceReceiveEvent info) {
        switch (info.orderStatus) {
            case OrderStatus.WAIT_PRODUCE_RECEIVE://大货样确认
     ...
     */
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onMessageEvent(WaitHandleProduceOrderReceiveRemitEvent info) {
        switch (info.orderStatus) {
            case OrderStatus.WAIT_PAY://待付款
...
//            不是第一页 就刷新
    ...
    public void onRefresh() {
        mPresenter.getWaitHandleProductOrderList(0, false);
    }
    ...
    public void onLoadMoreRequested() {
        if (data.hasNextPage) {
            ...
            //加载列表数据
            mPresenter.getWaitHandleProductOrderList(start, false);
        } else {
